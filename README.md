...
- Easy Mode: chosen on the start screen or from the header. One question per screen, very large buttons, short on-screen text, Back / Listen again / Continue, and "I don't know" wherever it is valid. It is a presentation layer only: the same engine picks the next question, and `tests/stretch.test.ts` checks that every sample household gets the same result in both modes (also shown live on `/review`).
- Read-aloud with option highlighting: the question and each option are separate utterances (or recorded Malayalam clips). An option is highlighted between its own start and end events (`lib/a11y/read-sequence.ts`). Speaking uses a dashed border, speaker icon and "Reading" label; selected uses a solid thick border, check mark and "Selected" label. Tapping stops speech and selects; Continue submits. Without speech the screen works by tap and says audio is unavailable.
- Document checklist: merged across potentially eligible schemes (`lib/checklist.ts`), expandable to show which schemes need each document, with "Official document list not published / not verified" where the dataset has none. Print or save as PDF through the browser's print dialog; the printed sheet holds scheme names, documents, where to apply and the disclaimer, and no answers or identity data. "Clear after printing" erases the session.
- Where to apply: office type comes from each scheme first, then the district or printed jurisdiction (`lib/offices.ts`). Location is asked only on "Find nearest centre" and stays on the device. No office has verified coordinates yet, so every office shows "Distance unavailable — official address provided" with Call and Directions links. Matsyafed districts without an office are reported and the two nearest listed offices are shown as other-district alternatives.
